...
const useResponsiveDimension = (responsive?: boolean, config?: any, key?: string) => {
  ...
  return responsive && key ? value : config[key!];
};
...
export function GradualBlur(props: GradualBlurProps) {
  ...
  const blurDivs = useMemo(() => {
    ...
    const currentStrength =
      isHovered && config.hoverIntensity ? baseStrength * config.hoverIntensity : baseStrength;
  ...
  const containerStyle = useMemo(() => {
    ...
    const zIndexBase = config.zIndex || 1000;
    ...
    const baseStyle: React.CSSProperties = {
      ...
      zIndex: isPageTarget ? zIndexBase + 100 : zIndexBase,
      ...config.style
    };
