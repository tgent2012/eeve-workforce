...
function FeatureCard({ children, step, steps }: { children: React.ReactNode; step: number; steps: readonly Step[] }) {
  ...
  return (
    ...
    >
      <div className="relative w-full overflow-hidden rounded-3xl border border-neutral-200/50 dark:border-white/10 bg-white/35 dark:bg-white/5 backdrop-blur-[12px] shadow-[0_8px_32px_0_rgba(0,0,0,0.05),inset_1px_1px_2px_rgba(255,255,255,0.3)] transition-colors duration-300">
        <div className="m-8 md:m-10 min-h-[500px] md:min-h-[450px] w-full flex flex-col md:flex-row gap-8 relative select-none">
          <AnimatePresence mode="wait">
            <motion.div
              key={step}
              className="flex w-full flex-col gap-4 md:w-5/12 text-left justify-center pr-4 z-20"
            ...
            >
              <motion.div
                className="text-xs font-bold uppercase tracking-widest text-[#A31D1D]"
                initial={{ opacity: 0, x: -20 }}
                animate={{ opacity: 1, x: 0 }}
              ...
              <motion.h3
                className="text-3xl font-sans font-black tracking-tight text-neutral-900 dark:text-neutral-100"
                initial={{ opacity: 0, x: -20 }}
                animate={{ opacity: 1, x: 0 }}
              ...
              >
                <p className="text-base leading-relaxed text-[#111111] dark:text-neutral-300 font-sans font-normal text-balance">
                  {steps[step].description}
                </p>
              </motion.div>
            </motion.div>
          </AnimatePresence>
          <div className="relative w-full md:w-7/12 h-[300px] md:h-full min-h-[300px] overflow-hidden">
            {children}
          </div>
        </div>
      </div>
                    ...
                    return (
                        <motion.li key={step.name} initial="inactive" animate={isCurrent ? "active" : "inactive"} variants={stepVariants} transition={{ duration: 0.3 }} className="relative" >
                            <button
                                type="button"
                                className={cn(
                                    "group flex items-center gap-2.5 rounded-full px-4 py-2 text-xs font-semibold uppercase tracking-wider transition-colors duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#A31D1D] dark:focus-visible:ring-offset-black",
                                    isCurrent 
                            ...
                            >
                                <span className={cn(
                                    "flex h-5 w-5 shrink-0 items-center justify-center rounded-full transition-all duration-300 text-[10px]",
                                    isCompleted 
                                        ? "bg-[#A31D1D] text-white" 
                                        : isCurrent 
...
const defaultClasses = {
  img: "rounded-2xl border border-neutral-200/60 dark:border-neutral-800/80 shadow-xl shadow-black/5 dark:shadow-neutral-950/40 object-cover",
  step1img1: "w-[65%] h-[75%] left-0 top-[5%]",
  step1img2: "w-[60%] h-[70%] left-[35%] top-[25%]",
  step2img1: "w-[65%] h-[75%] left-[5%] top-[5%]",
  step2img2: "w-[50%] h-[60%] left-[45%] top-[35%]",
  step3img: "w-[90%] h-[80%] left-[5%] top-[10%]",
  step4img: "w-[90%] h-[80%] left-[5%] top-[10%]",
} as const
      ...
      case 0:
        return (
          <div className="relative w-full h-full">
            ...
            <AnimatedStepImage alt={image.alt} className={cn(defaultClasses.img, step1img2Class)} src={image.step1img2} preset="slideInRight" delay={0.15} />
          </div>
        )
      case 1:
        return (
          <div className="relative w-full h-full">
            ...
            <AnimatedStepImage alt={image.alt} className={cn(defaultClasses.img, step2img2Class)} src={image.step2img2} preset="fadeInScale" delay={0.15} />
          </div>
        )
      case 2:
